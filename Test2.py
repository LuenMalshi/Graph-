...
def fake_data(x0, y0, t, freq):
    ...
    for i in range(nAntennas):
        ...
        s[i] = 1. / r * np.exp(-1j * 2. * np.pi * get_cyclotron_frequency(freq) * ( - r / c))
    return s
...
def get_cyclotron_frequency(cfreq):
    return cfreq

def get_phase(x0, y0, i, f):
	d = get_antenna_dist(x0, y0, i)
	return -2. * np.pi * get_cyclotron_frequency(f) * d / c
...
def shifted_sum(f, s, x0, y0, weighted=False):
    total_signal = 0
    normalization = 0 #s^2
    for i in range(nAntennas):
        phi_shift = get_phase(x0, y0, i, f)
        d = get_antenna_dist(x0, y0, i)
...
if __name__ == "__main__":
	...
	walength = []
	freqlist = []
	distlist = []               #will contain distance from center
	fulllist = []
	diff = []                   #list of difference between location with noise vs true location
	cf = 25.6e9                 #cyclotron frequency
	for x0 in np.arange (-0.04, 0.04, 0.01):
#		freqlist.append(cf)
#		walength.append(c/cf)
		sData = fake_data(x0,y0,time,cf) 
		kNoise = 50
		distlist.append(x0)
		sData = add_noise(sData, kNoise) + 1j * add_noise(sData, kNoise)       #data with noise
#	print(sData)
#	color map
		...
		for i in range(len(X)):
 		       for j in range(len(X[0])):
        		    if X[i][j]**2 + Y[i][j]**2 < 0.99*rAntennas**2:
       	     	 		   Z[i][j] = np.abs(sum(shifted_sum(cf, sData , X[i][j], Y[i][j] )))
	    #1
...
#	    	HD2 = []
		print max(D)
		print x0, X[ml][n], D[ml]
		diff.append(X[ml][n] - x0)
	plt.plot(distlist, diff)
	plt.show()
#--------------------------------------Code I recently commented out ----------------------------------------------------------------------
#	    	for i in range(len(X)):
#			if X[i][n] < x0:
...
#						Z2[i][j] = np.abs(sum(shifted_sum(cf,sData, X2[i][j], Y2[i][j])))	
#			ylist = Y2[0]
...
#						Z1[i][j] = np.abs(sum(shifted_sum(cf, sData, X1[i][j], Y1[i][j])))	
#			ylist = Y1[0]
...
#	print freqlist, fulllist
	#Plotting
#	plt.plot(walength, fulllist)
#	plt.show()
#--------------------------------------------------------------------------------------------------------------------------------------------
